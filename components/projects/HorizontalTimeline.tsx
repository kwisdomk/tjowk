...
function monthIndex(date: string): number {
  ...
  return months.indexOf(extractMonth(date));
}
  ...
  if (entry.projectId) {
    return (
      <Link 
        ...
        className="block h-full group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald focus-visible:ring-offset-2 focus-visible:ring-offset-black rounded-xl"
      >
        {content}
      </Link>
    );
...
function MonthColumn({ month, entries }: { month: string; entries: TimelineEntry[] }) {
  return (
    <div className="flex flex-col gap-4 min-w-[260px] max-w-[300px]">
      <h3 className="label-mono sticky left-0 text-emerald-dim font-bold">{month}</h3>
...
export function HorizontalTimeline({ timeline }: { timeline: TimelineEntry[] }) {
  ...
  return (
    <div 
      className="relative w-full overflow-x-auto overflow-y-hidden min-h-[360px] scrollbar-thin pb-6 -mx-6 px-6"
      role="region" 
      aria-label="Project timeline"
      tabIndex={0}
      style={{ WebkitOverflowScrolling: 'touch' }}
    >
      {/* Right fade-out gradient to indicate scrollability */}
      <div className="absolute right-0 top-0 bottom-0 w-16 bg-gradient-to-l from-black to-transparent pointer-events-none z-10 hidden md:block" />
      <div className="absolute left-0 top-0 bottom-0 w-6 bg-gradient-to-r from-black to-transparent pointer-events-none z-10 md:hidden" />
      <div className="absolute right-0 top-0 bottom-0 w-6 bg-gradient-to-l from-black to-transparent pointer-events-none z-10 md:hidden" />

      <div className="flex w-max min-w-full pt-8 pb-4 pr-12 relative z-0">
          ...
          return (
            <div 
              ...
              className={cn(
                ...
                isExpanded ? "flex-1 min-w-max pr-8" : "w-[120px] md:w-[200px]"
              )}
            >
              {/* Horizontal Axis */}
              ...
              <div className="relative z-10 flex items-center mb-8 h-7">
                <button
                  ...
                  className={cn(
                    "flex items-center gap-2 group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald focus-visible:ring-offset-2 focus-visible:ring-offset-black rounded-full pr-3 pl-1 py-1 transition-colors bg-black",
                    isExpanded ? "ring-1 ring-emerald-500/20 bg-emerald-glow/20" : "hover:bg-surface-2"
                  )}
                >
                  <div className={cn(
                    "w-4 h-4 rounded-full border flex items-center justify-center transition-colors",
                    isExpanded ? "border-emerald bg-emerald" : "border-border bg-surface group-hover:border-emerald-dim"
                  )}>
